...
def process_user_info(message):
    ...
    # التحقق مما إذا كان الإدخال آيدي رقمي أو يوزر نصي
    if user_input.isdigit():
        user_id = int(user_input)
    elif user_input.startswith("@"):
        user_id = user_input[1:]  # إزالة "@" من اليوزر
    else:
        bot.send_message(message.chat.id, "❌ يرجى إدخال آيدي رقمي أو يوزر بصيغة @username")
        return

    try:
        user = bot.get_chat(user_id)  # جلب معلومات المستخدم

        info_text = f"🆔 المعرف: {user.id}\n"
        info_text += f"👤 الاسم: {user.first_name} {user.last_name if user.last_name else ''}\n"
        info_text += f"📛 اسم المستخدم: @{user.username if user.username else 'لا يوجد'}\n"
        info_text += f"🌍 اللغة: {message.from_user.language_code}\n"

        bot.send_message(message.chat.id, info_text)
    except Exception as e:
        bot.send_message(message.chat.id, "❌ لم يتم العثور على المستخدم أو أن البوت لا يملك الصلاحية لعرض المعلومات.")

def get_user_info(user_identifier):
    """ جلب معلومات المستخدم باستخدام الأيدي أو اليوزر """
